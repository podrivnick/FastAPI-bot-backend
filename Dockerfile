...
WORKDIR /FastAPI-bot-backend

COPY --from=builder requirements.dev.txt /FastAPI-bot-backend
...
COPY ./config /FastAPI-bot-backend/config
COPY ./src /FastAPI-bot-backend/src
